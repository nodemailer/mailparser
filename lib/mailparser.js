 ...
 * 
 * <p>Splits on ";", the first value will be set as defaultValue property and will
 * not be handled, others will be split on "=" to key-value pairs</p>
 * 
 * <p>For example:</p>
 * 
...
MailParser.prototype._processMimeTree = function(){
    ...
    this._mailData = {html:[], text:[], alternatives:[], attachments:[]};
    ...
    if(this._mailData.html.length){
        for(i=0, len=this._mailData.html.length; i<len; i++){
            if(!returnValue.html || this._mailData.html[i].level < htmlLevel){
                if(html){
                ...
                htmlLevel = this._mailData.html[i].level;
                html = this._mailData.html[i].content;
            }
        }
    }
    
    if(this._mailData.text.length){
        for(i=0, len=this._mailData.text.length; i<len; i++){
            if(!returnValue.text || this._mailData.text[i].level < textLevel){
                if(text){
                    if(!returnValue.alternatives){
                        returnValue.alternatives = [];
                    }
                ...
                textLevel = this._mailData.text[i].level;
                text = this._mailData.text[i].content;
            }
        }
    }
    ...
    if(this._mailData.attachments.length){
        returnValue.attachments = [];
        for(i=0, len=this._mailData.attachments.length; i<len; i++){
            returnValue.attachments.push(this._mailData.attachments[i].content);
        }
    }
 ...
 * 
 * <p>Pushes the node into appropriate <code>this._mailData</code> array (<code>text/html</code> to <code>this._mailData.html</code> array etc)</p>
 * 
 * @param {Object} node A mime tree node
 * @param {Number} [level=0] current depth
 */
MailParser.prototype._processMimeNode = function(node, level){
    ...
    if(!node.attachment){
        switch(node.meta.contentType){
            case "text/html":
                this._mailData.html.push({content: node.content || "", level: level});
                break;
            case "text/plain":
                this._mailData.text.push({content: node.content || "", level: level});
                break;
        }
    }else{
        node.meta = node.meta || {};
        ...
        this._mailData.attachments.push({content: node.meta || {}, level: level});
    }  
};
