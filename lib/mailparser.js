...
MailParser.prototype._processHeaderLine = function(pos){
    ...
    switch(key){
        ...
        case "content-location":
            this._currentNode.meta.contentLocation = this._trimQuotes(value);
            break;
    }
...
MailParser.prototype._processMimeTree = function(){
    ...
    if(this.mailData.html.length){
        for(i=0, len=this.mailData.html.length; i<len; i++){
            if(!returnValue.html || this.mailData.html[i].level < htmlLevel){
                ...
                if(this.mailData.html[i].contentLocation){
                    returnValue.contentLocation = this.mailData.html[i].contentLocation;
                }
            }else{
                if(!returnValue.alternatives){
                    returnValue.alternatives = [];
    ...
    if(returnValue.contentLocation){
        this._processMHTContentLocation(returnValue);
    }
...
/**
 * <p>Replaces content-location values with content-id values (if needed)</p>
 *
 * @param {Object} returnValue Value to be returned with "end"
 */
MailParser.prototype._processMHTContentLocation = function(returnValue){
    var base = returnValue.contentLocation || "",
        basePath,
        attachmentList = [],
        attachment,
        absolutePath,
    ...
    for(var i=0, len = (returnValue.attachments || []).length; i<len; i++){
        attachment = returnValue.attachments[i];
        if(attachment.contentLocation){
            absolutePath = attachment.contentLocation;

            if(!attachment.contentId){
                attachment.contentId = "mht-" + (++counter) + "@tmp";
            }
            
            if(attachment.contentLocation.indexOf(basePath) === 0){
                relativePath = absolutePath.substr(basePath.length);
            }else{
                relativePath = false;
            }

            returnValue.html = returnValue.html.
                replace(new RegExp("\\b"+absolutePath.replace(/([\\^$*+?.():=!|{},\[\]])/g, "\\$1")+"\\b", "g"), "cid:"+attachment.contentId).
                replace(new RegExp("\\b"+relativePath.replace(/([\\^$*+?.():=!|{},\[\]])/g, "\\$1")+"\\b", "g"), "cid:"+attachment.contentId);
    ...
    if(!node.attachment){
        switch(node.meta.contentType){
            case "text/html":
                ...
                this.mailData.html.push({content: this._updateHTMLCharset(node.content || ""), level: level, contentLocation: node.meta.contentLocation});
                return;
            case "text/plain":
